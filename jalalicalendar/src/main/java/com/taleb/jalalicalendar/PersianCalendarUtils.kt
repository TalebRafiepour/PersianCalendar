...
object PersianCalendarUtils {
     ...
     * the number of days since January one in the year 4713 BC). Since the
     ...
     * corresponds with one Farvardin (one) one.
     *
     * @param year
     * int persian year
     * @param month
    ...
    fun persianToJulian(year: Long, month: Int, day: Int): Long {
        return 365L * (ceil((year - 474L).toDouble(), 2820.0) + 474L - 1L) + Math.floor((682L * (ceil((year - 474L).toDouble(), 2820.0) + 474L) - 110L) / 2816.0).toLong() + (PersianCalendarConstants.PERSIAN_EPOCH - 1L) + 1029983L * Math.floor((year - 474L) / 2820.0).toLong() + (if (month < 7) 31 * month else 30 * month + 6).toLong() + day.toLong()
    }
    ...
    fun isPersianLeapYear(persianYear: Int): Boolean {
        return ceil((38.0 + (ceil((persianYear - 474L).toDouble(), 2820.0) + 474L)) * 682.0, 2816.0) < 682L
    }

    /**
     * Converts a provided Julian Day Number (i.e. the number of days since
     * January one in the year 4713 BC) to the Persian (Shamsi) date. Since the
     * Persian calendar is a highly regular calendar, converting to and from a
     * Julian Day Number is not as difficult as it looks. Basically it's a
    ...
    fun julianToPersian(julianDate: Long): Long {
        ...
        val ycycle = if (cyear != 1029982L) Math.floor((2816.0 * cyear.toDouble() + 1031337.0) / 1028522.0).toLong() else 2820L
        ...
        val month = (if (aux > 186L) Math.ceil((aux - 6L).toDouble() / 30.0) - 1 else Math.ceil(aux.toDouble() / 31.0) - 1).toInt()
